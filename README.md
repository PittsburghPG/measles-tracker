...
2. If a county's case count went up, it adds a new row to `measles_daily_county.tsv` for that increase, recording that county's running case total for the year (across both outbreaks) as `cumulative_cases`
